...
-- ============================================
-- 1. Create product_image_revisions table
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_product_image_revisions_product_idea ON product_image_revisions(product_idea_id);
CREATE INDEX IF NOT EXISTS idx_product_image_revisions_user ON product_image_revisions(user_id);
CREATE INDEX IF NOT EXISTS idx_product_image_revisions_view_type ON product_image_revisions(view_type);
CREATE INDEX IF NOT EXISTS idx_product_image_revisions_active ON product_image_revisions(is_active);
CREATE INDEX IF NOT EXISTS idx_product_image_revisions_created ON product_image_revisions(created_at DESC);

-- Create composite unique constraint for active revisions
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_active_revision ON product_image_revisions(product_idea_id, view_type, is_active) 
...
-- Drop trigger if exists and recreate
DROP TRIGGER IF EXISTS manage_active_revision_trigger ON product_image_revisions;
...
-- ============================================
SELECT 
  'product_image_revisions table created successfully' as status,
  COUNT(*) as column_count
FROM information_schema.columns 
WHERE table_name = 'product_image_revisions';
