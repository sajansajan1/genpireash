...
import { analyzeProductViews, analyzeImage } from "@/lib/services/image-analysis-service";

/**
 * Analyze and save analysis for newly generated product images
 */
export async function analyzeAndSaveProductImages(
  productId: string,
  images: {
  ...
  try {
    console.log(`Analyzing images for product ${productId}${revisionId ? ` (revision: ${revisionId})` : ''}`);
    ...
    for (const [viewType, imageData] of Object.entries(images)) {
      if (imageData && !['front', 'back', 'side'].includes(viewType)) {
        ...
        if (imageUrl) {
          ...
          await analyzeImage(
            imageUrl,
            `${productName || "Product"} - ${viewType} view`,
            additionalContext
          );
        }
      }
 ...
 */
export async function getProductImageAnalysis(
  productId: string
): Promise<{
  success: boolean;
  analyses?: any[];
...
export async function refreshImageAnalysis(
  ...
  try {
    ...
    if (productId) {
      await supabase
        .from("image_analysis_cache")
        .update({ product_idea_id: productId })
        .eq("image_url", imageUrl);
    }
