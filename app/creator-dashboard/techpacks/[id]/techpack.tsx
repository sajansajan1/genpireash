...
export default function TechPackDetailPage({ id }: { id: string }) {
  ...
  const { getTechPack, loadingGetTechPack, errorGetTechPack, refreshGetTechPack, setGetTechPack } =
    useGetTechPackStore();
  const { setUpdateTechPack } = useUpdateTechPackStore();
  ...
  const [svgLoader, setSvgLoader] = useState<"technical" | "multiview" | "construction" | "callout" | null>(null);

  const [pngLoader, setPngLoader] = useState<boolean>(false);
  const [costView, setCostView] = useState<"sample" | "production">("sample");
  ...
  const { refresUserNotifications } = useGetNotificationsStore();
  ...
  useEffect(() => {
    const loadTechpack = async () => {
      if (id) {
        await refreshGetTechPack({ id });
      }
    };
    loadTechpack();
  }, [id, refreshGetTechPack]);
  ...
  if (!getTechPack || loadingGetTechPack || errorGetTechPack) {
    return (
  ...
  console.log(getTechPack, "getetecds");
  ...
  const generateImages = async (imageType: "vector" | "detail" | "technical" | "all" = "all") => {
    if (getTechPack?.tech_pack?.metadata) {
      toast({
        variant: "destructive",
        title: "No Tech Pack Available!",
        description: "Please generate a Tech Pack first to download better technical images for improved results.",
      });
      return;
    }
    if ((credits?.credits ?? 0) < 6) {
      return toast({
        variant: "destructive",
        title: "No Credits left!",
        description: "You don't have any credits left. Please purchase Credits to Generate Technical Files",
      });
    }
    setImageLoader(true);
    try {
      const response = await fetch("/api/product-pack-generation/generate-techpack-images", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          techPackData: getTechPack,
          imageType,
        }),
      });
      ...
      if (result.success) {
        const deduct = await DeductCredits({ credit: 6 });
        await refreshGetTechPack({ id });
        await setCreateNotification({
          senderID: user?.id,
          receiverID: user?.id,
          title: "Your product is ready ✅",
          message: `Well done, ${user?.full_name}! Your production guidelines and technical files are complete. Open your product page to download and share them with manufacturers.`,
          type: "rfq_response",
        });
        await refresUserNotifications();
        await refetch();
  ...
  const handleGenerate3D = async () => {
    // Check credits
    if ((credits?.credits ?? 0) < 10) {
      return toast({
        variant: "destructive",
        title: "Insufficient Credits",
        description: "You need 10 credits to generate a 3D model.",
      });
    }

    // Check if we have the required images
    if (!getTechPack.image_data?.front?.url || !getTechPack.image_data?.back?.url) {
      return toast({
        variant: "destructive",
        title: "Missing Images",
        description: "Please ensure you have at least front and back view images.",
      });
    }
    ...
    try {
      ...
      const response = await fetch("/api/generate-3d-model", {
        ...
        body: JSON.stringify({
          imageUrls: {
            front: getTechPack.image_data?.front?.url,
            back: getTechPack.image_data?.back?.url,
            side: getTechPack.image_data?.side?.url,
            top: getTechPack.image_data?.top?.url,
            bottom: getTechPack.image_data?.bottom?.url,
          },
          dimensions: getTechPack.tech_pack?.dimensions,
        }),
      });
      ...
      const pollInterval = setInterval(async () => {
        try {
          const statusRes = await fetch(`/api/generate-3d-model?taskId=${taskId}`);
      ...
      // Set timeout to prevent infinite polling (5 minutes max)
      setTimeout(() => {
        clearInterval(pollInterval);
        if (is3DGenerating) {
          setIs3DGenerating(false);
  ...
  const handleUpdateSection = async (sectionKey: string, newContent: any) => {
    ...
    try {
      const updatedData = {
        ...getTechPack.tech_pack,
        [sectionKey]: newContent,
      };

      const { success, data, error } = await setUpdateTechPack(id, updatedData);

      if (success && data) {
        setGetTechPack({
          ...getTechPack,
          tech_pack: data.tech_pack || updatedData,
        });
  ...
  const generateColorVariations = async () => {
    setIsGeneratingVariations(true); // Placeholder for AI color variation generation logic
    setTimeout(() => {
      const generatedVariations = [
        { name: "Teal", primaryColor: "#008080" },
        { name: "Maroon", primaryColor: "#800000" },
        { name: "Olive", primaryColor: "#808000" },
      ];
      setVariations([...variations, ...generatedVariations]);
  ...
  const downloadSvg = async (
    ...
    loaderType: "technical" | "multiview" | "construction" | "callout"
  ) => {
    const hasSubscriptionHistory = credits?.membership === "pro";

    if (!hasSubscriptionHistory) {
      toast({
        title: "Subscription Required",
        description: "You need a pro plan to generate SVG Images. Please subscribe to continue.",
        variant: "destructive",
      });
      return null;
    }
    ...
    if (getTechPack?.tech_pack?.metadata) {
      toast({
        variant: "destructive",
        title: "No Tech Pack Available!",
        description: "Generate a Tech Pack first to download the SVGs.",
      });
      return null;
    }

    const imageUrls = Array.isArray(images) ? images.filter(Boolean) : [images].filter(Boolean);

    if (!imageUrls.length) {
      toast({
        variant: "destructive",
        title: "No Technical Images Available!",
        description: "Please generate at least one technical image before downloading SVGs.",
      });
      return null;
    }

    try {
      setSvgLoader(loaderType);
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        let base64 = item.svgText || item.svgBase64;
        if (!base64) continue;
        ...
        zip.file(`converted-image-${i + 1}.svg`, svgDecoded);
      }
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${getTechPack?.tech_pack?.productName || "svg-files"}.zip`;
      ...
      toast({
        title: "SVGs Downloaded!",
        description: `Downloaded SVG file(s) successfully.`,
        variant: "default",
      });
    } catch (err: any) {
      console.error(err?.message || "A network error occurred.");
      toast({
        variant: "destructive",
        title: "Network Error",
        description: err?.message || "Failed to download SVGs.",
      });
    } finally {
      setSvgLoader(null);
    }
  ...
  const handleGenerateTechPack = async () => {
    ...
    try {
      ...
      const result = await generateTechPackForProduct(id);
      ...
      if (result.success) {
        ...
        const productData = await getUserProjectIdea(id);
        if (productData && productData.tech_pack && Object.keys(productData.tech_pack).length > 0) {
          // Update the main tech pack data state

          toast({
            variant: "default",
            title: "Tech Pack Generated!",
            description: "Your technical specifications have been created successfully.",
          });
          await refetch();
          ...
          toast({
            variant: "default",
            title: "Tech Pack Generated!",
            description: "Your technical specifications have been created. Refreshing...",
          });
        ...
        toast({
          variant: "destructive",
          title: "Generation Failed",
          description: result.error || "Failed to generate tech pack. Please try again.",
        });
      ...
      >
        {!getTechPack.tech_pack?.metadata && (
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => router.push(`/ai-designer?projectId=${id}&version=modular`)}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                Product Editor
              </DropdownMenuItem>
              <DropdownMenuItem onClick={(e) => router.push(`/product/${id}`)}>
        ...
        <div className="flex flex-col space-y-2 gap-2 lg:flex-row sm:space-y-0 sm:space-x-2">
          {!getTechPack?.tech_pack?.metadata && (
            <>
              ...
              {/* <Button variant="outline" onClick={() => downloadSvg(getTechPack?.technical_images?.technicalImage?.url)}>
                {svgLoader ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
                    Generating SVG...
          ...
          {getTechPack.tech_pack?.metadata && (
            <Button onClick={handleGenerateTechPack} disabled={isGeneratingTechPack}>
        ...
        {isEditingProductName ? (
          <form
            onSubmit={(e) => {
              ...
              if (formData.productName !== getTechPack.tech_pack.productName) {
                handleUpdateSection("productName", formData.productName);
              }
            }}
          >
            ...
            <Card>
              ...
              <CardContent>
                ...
                {/* <div className="pt-6 border-t border-gray-200 dark:border-gray-700 mt-4">
                  <div className="flex flex-row items-center justify-between mb-4 md:flex-row gap-2">
                    <h3 className="text-lg font-semibold">Product Variations</h3>

                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <Button variant="outline" size="sm" onClick={() => setShowAddVariation(true)}>
                        Add Variation
                      </Button>
                      ...
                      >
                        {isGeneratingVariations ? "Generating…" : "AI Color Variations"}
                      </Button>
                    </div>
                  </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <div className="flex gap-2 mt-1">
                              <Button
                                type="button"
                                variant={newVariation.colorCategory === "primary" ? "default" : "outline"}
                                size="sm"
                                onClick={() =>
                                  setNewVariation({
                                    ...newVariation,
                              ...
                              <Button
                                type="button"
                                variant={newVariation.colorCategory === "accent" ? "default" : "outline"}
                                size="sm"
                                onClick={() =>
                                  setNewVariation({
                                    ...newVariation,
                          ...
                          <div>
                            <Label htmlFor="variation-primary-color">Primary Color</Label>
                            <div className="flex gap-2">
                              <Input
                                id="variation-primary-color"
                                value={newVariation.primaryColor}
                                onChange={(e) =>
                                  setNewVariation({
                                    ...newVariation,
                          ...
                          {newVariation.colorType === "two-tone" && (
                            <div>
                              <Label htmlFor="variation-secondary-color">Accent Color</Label>
                              <div className="flex gap-2">
                                <Input
                                  id="variation-secondary-color"
                                  value={newVariation.secondaryColor}
                                  onChange={(e) =>
                                    setNewVariation({
                                      ...newVariation,
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              disabled={
                                !newVariation.name ||
                                !newVariation.primaryColor ||
                                (newVariation.colorType === "two-tone" && !newVariation.secondaryColor)
                              }
                              onClick={async () => {
                                ...
                                const { colors } = getTechPack.tech_pack;
                                const updatedContent = {
                                  primaryColors: colors.primaryColors ?? [],
                                  accentColors: colors.accentColors ?? [],
                                  styleNotes: colors.styleNotes ?? "",
                                  trendAlignment: colors.trendAlignment ?? "",
                                };

                                await handleUpdateSection("colors", updatedContent);
                            ...
                            <Button variant="outline" size="sm" onClick={() => setShowAddVariation(false)}>
                              Cancel
                            </Button>
                          </div>
                        </div>
                  ...
                  <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                    <p className="text-xs text-[#1C1917] leading-relaxed">
                      {costView === "sample"
                        ? "Sample costs include specialized tooling, engineering consultation, and premium materials for prototype development. Molding and pattern making are one-time setup costs."
                        : "Production estimates are based on 1000-unit minimum orders with standard lead times. Actual costs may vary based on material availability, seasonal demand, and supplier negotiations."}
                    </p>
                  </div>
                </div> */}
              </CardContent>
              ...
              <CardContent>
                      ...
                      {!getTechPack?.tech_pack?.metadata && (
                        <div className="flex flex-col md:flex-row gap-4">
                          <Button
                            ...
                            variant="default"
                            onClick={() => generateImages("all")}
                          >
                            <div className="flex items-center justify-center gap-2">
                              <FileText className="h-4 w-4 shrink-0" />

                              <span className="sm:hidden">Generate File</span>
                      ...
                      <DropdownMenuContent>
                        ...
                        >
                          {svgLoader === "technical" ? (
                            ...
                            <>
                              <BookImage className="h-4 w-4 " /> SVG
                            </>
                          )}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                ...
                >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem
                              ...
                              onClick={() =>
                                downloadSvg([getTechPack.technical_images?.constructionImage?.url], "construction")
                              }
                            >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem
                              ...
                              onClick={() => downloadSvg([getTechPack.technical_images?.calloutImage?.url], "callout")}
                            >
                              {svgLoader === "callout" ? (
                                <>
                                  <Loader2 className="h-4 w-4 animate-spin" />
                  ...
                  {/* <AccordionItem value="item-4">
                    <AccordionTrigger>4. Dimensions & Measurements - Precision Sizing</AccordionTrigger>
                    <AccordionContent>
                      <div className="mb-4 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        <h4 className="font-medium text-orange-900 mb-2">Importance of Accurate Measurements</h4>
                        <p className="text-sm text-orange-800">
                          Precise measurements are the foundation of successful manufacturing. Component-based
                          measurements ensure each part fits perfectly together, reducing waste and improving production
                          efficiency.
                        </p>
                      </div>
                      <div
                        className="bg-gray-50 p-4 rounded border-2 border-dashed mb-4"
                        onClick={() =>
                          setModalImage(getTechPack.technical_images?.measurementImage?.url || "/placeholder.svg")
                        }
                      >
                        <img
                          src={getTechPack.technical_images?.measurementImage?.url || "/placeholder.svg"}
                          alt="Measurement indicators view"
                          className="object-cover w-full h-full mb-2"
                        />
                        <h5 className="text-xs font-medium text-gray-700 mb-1">Component Measurements</h5>
                        <p className="text-xs text-gray-600">
                          Alphabetical indicators (A, B, C...) marking each measurable component
                        </p>
                      </div>

                      {getTechPack.technical_images?.componentMeasurements && (
                        <ComponentMeasurementGrid
                          data={getTechPack.technical_images.componentMeasurements}
                          className="mt-4"
                        />
                      )}
                      ...
                      {!getTechPack.technical_images?.componentMeasurements && (
                        <div className="mt-4 p-4 bg-muted rounded-lg">
                          <p className="text-sm text-[#1C1917]">
                            Component measurements will be displayed here after generation - showing width, height,
                            length, depth, and weight for each labeled component
                          </p>
                        </div>
                      )}
                      ...
                      <ul className="space-y-2 text-sm mt-4">
                        <li>
                          • <strong>Component labeling</strong> – Each letter (A, B, C...) marks a specific measurable
                          part
                        </li>
                        <li>
                          • <strong>Comprehensive data</strong> – Width, height, length, depth, and weight for each
                          component
                        </li>
                        <li>
                          • <strong>Professional documentation</strong> – Industry-standard measurement presentation for
                          manufacturers
                        </li>
                        <li>
                          • <strong>Quality control</strong> – Enables precise pattern making and size grading
                        </li>
                      </ul>
                    </AccordionContent>
                  </AccordionItem> */}
                  ...
                  {/* <AccordionItem value="item-5">
                    <AccordionTrigger>5. Scale & Proportion - Visual Accuracy</AccordionTrigger>
                    <AccordionContent>
                      <div className="mb-4 p-3 bg-teal-50 rounded-lg border border-teal-200">
                        <h4 className="font-medium text-teal-900 mb-2">Why Scale Matters</h4>
                        <p className="text-sm text-teal-800">
                          Accurate scale and proportion indicators ensure manufacturers understand the relative size of
                          design elements. This prevents misinterpretation that could lead to oversized or undersized
                          components.
                        </p>
                      </div>
                      <div
                        className="bg-gray-50 p-4 rounded border-2 border-dashed mb-4"
                        onClick={() =>
                          setModalImage(getTechPack.technical_images?.scaleProportionImage?.url || "/placeholder.svg")
                        }
                      >
                        <img
                          src={getTechPack.technical_images?.scaleProportionImage?.url || "/placeholder.svg"}
                          alt="Scale and proportion view"
                          className="object-cover w-full h-full mb-2"
                        />
                        <h5 className="text-xs font-medium text-gray-700 mb-1">Proportion References</h5>
                        <p className="text-xs text-gray-600">
                          Visual scale indicators showing relative sizing of design elements
                        </p>
                      </div>
                      <ul className="space-y-2 text-sm">
                        <li>
                          • <strong>Proportional accuracy</strong> – Ensures design elements maintain correct size
                          relationships
                        </li>
                        <li>
                          • <strong>Scale references</strong> – Optional ruler or grid system for size verification
                        </li>
                        <li>
                          • <strong>Visual consistency</strong> – Maintains design integrity across different production
                          sizes
                        </li>
                        <li>
                          • <strong>Manufacturing clarity</strong> – Reduces interpretation errors during production
                          setup
                        </li>
                      </ul>
                    </AccordionContent>
          ...
          {model3D && (
            <div className="space-y-4 sm:space-y-6 p-4 sm:p-0">
              ...
              <div className="pt-4 border-t">
                <ExportOptions modelUrl={model3D.url} techPackName={getTechPack.tech_pack?.productName || "product"} />
              </div>
            </div>
          )}
        </DialogContent>
