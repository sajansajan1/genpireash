/**
 * Admin API Route: Migrate Component Constraint
 * This endpoint adds 'component' to the valid_file_type constraint
 *
 * WARNING: This is a temporary admin endpoint. Remove after migration is complete.
 *
 * To use: Navigate to http://localhost:3002/api/admin/migrate-component-constraint
 */
...
export async function GET(request: NextRequest) {
  try {
    console.log('[Migration] Starting component constraint migration...');
    ...
    const migrationSteps = {
      ...
      sql: `
-- Add 'component' to valid_file_type constraint in tech_files table
...
ADD CONSTRAINT valid_file_type CHECK (file_type IN (
  ...
  'complete_pack'
    ...
    // Try to test if component type is allowed
    console.log('[Migration] Testing if component type is currently allowed...');
    ...
    if (insertError) {
      if (insertError.message.includes('violates check constraint "valid_file_type"')) {
        return NextResponse.json({
          success: false,
          migrationNeeded: true,
          status: 'Component type NOT allowed - Migration REQUIRED',
        ...
        return NextResponse.json({
          ...
          status: 'Component type IS allowed - Migration already applied! ✓',
          message: 'The database constraint already includes "component" file type.',
    ...
    return NextResponse.json({
      ...
      migrationNeeded: false,
      status: 'Component type IS allowed - Migration already applied! ✓',
      message: 'Test insert succeeded. The database is ready for component storage.',
    }, { status: 200 });
