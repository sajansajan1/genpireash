/**
 * Admin API Route: Migrate Flat Sketch Constraint
 * This endpoint adds 'flat_sketch' to the valid_file_type constraint
 *
 * WARNING: This is a temporary admin endpoint. Remove after migration is complete.
 *
 * To use: Navigate to http://localhost:3002/api/admin/migrate-flat-sketch-constraint
 */
...
export async function GET(request: NextRequest) {
  try {
    console.log("[Migration] Starting flat_sketch constraint migration...");
    ...
    const migrationSteps = {
      ...
      sql: `
-- Add 'flat_sketch' to valid_file_type constraint in tech_files table
...
ADD CONSTRAINT valid_file_type CHECK (file_type IN (
  ...
  'complete_pack'
    ...
    // Try to test if flat_sketch type is allowed
    console.log(
      "[Migration] Testing if flat_sketch type is currently allowed..."
    );
        ...
        return NextResponse.json(
          {
            ...
            status: "flat_sketch type NOT allowed - Migration REQUIRED",
        ...
        return NextResponse.json(
          {
            ...
            status:
              "flat_sketch type IS allowed - Migration already applied!",
            message:
              'The database constraint already includes "flat_sketch" file type.',
    ...
    return NextResponse.json(
      {
        success: true,
        migrationNeeded: false,
        status: "flat_sketch type IS allowed - Migration already applied!",
        message:
          "Test insert succeeded. The database is ready for flat_sketch storage.",
      },
      { status: 200 }
    );
  } catch (error) {
